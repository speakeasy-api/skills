...
@click.option("--model", default="claude-sonnet-4-20250514", help="Model to use for evaluation")
...
def compare(suite: str, model: str):
    ...
    with console.status("[yellow]Testing base model..."):
        results_no_skills = asyncio.run(runner.run(suite=suite, with_skills=False))

    console.print("\n[bold]Running WITH skills...[/bold]")
    with console.status("[green]Testing with skills..."):
        results_with_skills = asyncio.run(runner.run(suite=suite, with_skills=True))

    reporter = Reporter(console)
    reporter.print_comparison(results_no_skills, results_with_skills)
...
def list_tests():
    ...
    for suite, cases in tests.items():
        ...
        for case in cases[:5]:
            console.print(f"  - {case['skill']}: {case.get('prompt', case.get('should_activate', ['...'])[0])[:50]}...")
        if len(cases) > 5:
